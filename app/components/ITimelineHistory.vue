...
const { data, pending, error } = await useAsyncData<{ data: Slider }>(
  () => `slider:${name.value || 'unknown'}`,
  () =>
    isliderSliderRepository.show(name.value, {
      include: 'slides.translations,slides.files',
      filter: { field: 'system_name' }
    })
)

const items = computed(() => data.value?.data.slides ?? [])
  ...
  <div class="bg-history">
    <UContainer>
      ...
      <div class="text-center pt-20 pb-10">
        <h2 v-if="title" class="font-bold text-[30px] lg:text-[40px]">{{ title }}</h2>
        <p  v-if="subtitle" class="mt-2 text-[16px] lg:text-[20px]"> {{ subtitle }} </p>
      </div>
              ...
              <div class="relative ">
                ...
                <span class="hidden md:block absolute top-13 -right-[58px]"
                  ...
                  <span class="relative flex h-3 w-3">
                    <span class="absolute inline-flex h-full w-full rounded-full bg-tertiary"></span>
                    <span class="relative inline-flex h-3 w-3 rounded-full bg-tertiary"></span>
                  </span>
                </span>

                <!-- Cabecera año + foto -->
                <div class="flex gap-4">
                  <IMediaRender :media="item.files.slideimage" :alt="item.title" aspect-ratio="aspect-square"
                  ...
                  <div class="timeline-text-info">
                    ...
                    <p v-if="item.summary" class="mt-1 text-sm sm:text-[15px] leading-relaxed text-gray-600"> {{  item.summary  }} </p>
                    <div v-else class="mt-1 text-sm sm:text-[15px] leading-relaxed text-gray-600 custom-html" v-html="item.customHtml" ></div>
                  </div>
                </div>
